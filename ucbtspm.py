    ...
    def get_action(self, t):
        ...
        if t < self.N:
            ...
            if np.random.choice([True, False], size = 1, p = [epsilon,1-epsilon]):
                ...
                hat_mu = self.game.LossMatrix @ p_tilde
                lcb = hat_mu - np.sqrt( 2 * np.log(t+1) / np.sum(self.n, 1)  )
                action = np.argmin(  lcb  )
            ...
            # print('hat_mu:', hat_mu, ' bound: ', np.sqrt( 2 * np.log(t+1) / np.sum(self.n, 1)  ), ' lcb: ', lcb , ' action: ', action )
        return action
