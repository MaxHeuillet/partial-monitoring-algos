...
class CPB_gaussian():
    ...
    def __init__(self, game, horizon, alpha, with_f2 ):
    ...
    def obtain_probability(self, t):
        ...
        alphas = np.arange(0, U, U/M )

        p_m_hat =  np.array([ np.exp( (-1 * alphas[i]**2) / ( 2* sigma**2 )  )  for i in range(M-1) ] )
    ...
    def obtain_probability2(self, t):

        epsilon = 10e-7
        M = 20
        sigma = 1/16
        U = ( self.alpha  * np.log(t) ) ** 1/3
        alphas = np.arange(0, U, U/M )
    ...
    def get_action(self, t):
        ...
        if t <  1 * self.N:
            ...
            for k in range(self.N):
                Z2 = self.obtain_probability2(t)
            
                if self.with_f2:
                    if self.n[k] <=  self.eta[k] * geometry_v3.f_v2(t, self.alpha, Z2) :
                        R_t.append(k)
                else:
