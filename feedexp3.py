...
class FeedExp3():
    ...
    def __init__(self, game, horizon, method):
        ...
        self.C = pow( self.k_star * np.sqrt(np.exp(1.) - 2.), 2./3.)
    ...
    def get_action(self, t):
        ...
        if self.method == 'Bianchi':
            self.eta, self.gamma = self.parameters_Bianchi(self.C, t+1) 
            
        self.pbt_hat =  (1 - self.gamma) * self.pbt  + self.gamma * self.u 
        action = np.random.choice(self.game.n_actions, 1,  p = self.pbt )[0]
        return action

    def update(self, game, action, feedback):

        x = np.array( [ feedback * game.LinkMatrix[i][action] / self.pbt_hat[i] for i in range(game.n_actions) ] )
    ...
    def parameters_Bianchi(self, C, t):
        ...
        if t == 1: 
            eta = 1 / C * pow( np.log( self.game.n_actions ) / ( self.game.n_actions * t ) , 2./3.) 
            gamma = min(1,  C * pow( ( np.log( self.game.n_actions ) * self.game.n_actions **2) / t , 1./3.) )
        else: 
            eta = 1 / C * pow( np.log( self.game.n_actions ) / ( self.game.n_actions * t ) , 2./3.) 
            gamma = C * pow( ( np.log( self.game.n_actions ) * self.game.n_actions **2) / t , 1./3.)
        return eta, gamma 
